...
if (env.train):
    ...
    for epoch in range(1, number_epochs):
        ...
        PlotData['AI'].append(int("{:,.0f}".format(env.total_energy_ai)))
        PlotData['NOAI'].append(int("{:,.0f}".format(env.total_energy_noai)))
        ...
        # SAVING THE MODEL
        model.save("model.h5")
Plot_List = [*range(1, epoch+1, 1)]
index = np.array(Plot_List)

dataFrame = pd.DataFrame(data=PlotData, index=index);
dataFrame.plot.bar(rot=15, title="Energy Consumption Comparison between AI and Non AI");
